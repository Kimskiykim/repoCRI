...
{
	std::cout << std::endl
			  << "prefix = " << msg.getPrefix() << ", command = " << msg.getCommand();
...
bool isValidNick(const std::string &nick)
{
	...
	{
		if ((nick[i] >= 'a' && nick[i] <= 'z') || (nick[i] >= 'A' && nick[i] <= 'Z') || (nick[i] >= '0' && nick[i] <= '9') || (special.find(nick[i]) != std::string::npos))
			continue;
		else
			return (false);
	}
...
bool isValidChannelName(const std::string &name)
{
	...
	{
		if (name[i] == ' ' || name[i] == 7 || name[i] == 0 || name[i] == 13 || name[i] == 10 || name[i] == ',')
			return false;
	}
	return true;
}
